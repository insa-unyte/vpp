/*
 * delayprobe.c - skeleton vpp-api-test plug-in
 *
...
#define __plugin_msg_base delayprobe_test_main.msg_id_base
...
typedef struct
{
  ...
  /** vat_main_t pointer */
  vat_main_t *vat_main;
} delayprobe_test_main_t;

delayprobe_test_main_t delayprobe_test_main;

static int
api_delayprobe_tx_interface_add_del (vat_main_t *vam)
{
  ...
  M (DELAYPROBE_TX_INTERFACE_ADD_DEL, mp);
...
static int
api_delayprobe_interface_add_del (vat_main_t *vam)
{
  unformat_input_t *i = vam->input;
  int enable_disable = 1;
  ...
  /* Construct the API message */
  M (DELAYPROBE_INTERFACE_ADD_DEL, mp);
  mp->sw_if_index = ntohl (sw_if_index);
  mp->is_add = enable_disable;
  mp->which = which;
...
static int
api_delayprobe_interface_dump (vat_main_t *vam)
{
  unformat_input_t *i = vam->input;
  ...
  /* Construct the API message */
  M (DELAYPROBE_INTERFACE_DUMP, mp);
  ...
  PING (&delayprobe_test_main, mp_ping);
  S (mp_ping);
...
static void
vl_api_delayprobe_interface_details_t_handler (
  vl_api_delayprobe_interface_details_t *mp)
{
  vat_main_t *vam = delayprobe_test_main.vat_main;
  u32 sw_if_index;
  u8 which;
  u8 direction;
  u8 *out = 0;
...
static int
api_delayprobe_params (vat_main_t *vam)
{
  unformat_input_t *i = vam->input;
  u32 active_timer = ~0;
  u32 passive_timer = ~0;
  vl_api_delayprobe_params_t *mp;
  int ret;
  u8 record_flags = 0;
...
static int
api_delayprobe_set_params (vat_main_t *vam)
{
  unformat_input_t *i = vam->input;
  vl_api_delayprobe_set_params_t *mp;
  u32 active_timer = ~0;
  u32 passive_timer = ~0;
  u8 record_flags = 0;
  int ret;
...
static int
api_delayprobe_get_params (vat_main_t *vam)
{
  vl_api_delayprobe_get_params_t *mp;
  int ret;

  /* Construct the API message */
  M (DELAYPROBE_GET_PARAMS, mp);
...
static void
vl_api_delayprobe_get_params_reply_t_handler (
  vl_api_delayprobe_get_params_reply_t *mp)
{
  vat_main_t *vam = delayprobe_test_main.vat_main;
  u8 *out = 0;
 ...
 */
#include <delayprobe/delayprobe.api_test.c>
